...
class GdbSrvController::GdbSrvControllerImpl
{
    ...
    //      +
    //
    SimpleCharBuffer GdbSrvControllerImpl::ReadMemory(_In_ AddressType address, _In_ size_t maxSize, _In_ bool isPhysicalMemory)
    {
        ...
        {   
            ...
            for (;;)
            {
                ...
                PCSTR pFormat = GetReadMemoryCmd(isPhysicalMemory);
    ...
    PCSTR GetReadMemoryCmd(_In_ bool isPhysicalMemory)
    {
        PCSTR pFormat;
        
        if (isPhysicalMemory && m_pRspClient->IsFeatureEnabled(PACKET_READ_TRACE32_PHYSICAL_MEM))
        {
             pFormat = (m_targetProcessorArch == ARM64_ARCH || m_targetProcessorArch == AMD64_ARCH) ?
                       "qtrace32.memory:a,%I64x,%x" : "qtrace32.memory:a,%x,%x";
        }
        else
        {
             pFormat = (m_targetProcessorArch == ARM64_ARCH || m_targetProcessorArch == AMD64_ARCH) ?
...
SimpleCharBuffer GdbSrvController::ReadMemory(_In_ AddressType address, _In_ size_t size, _In_ bool isPhysicalMemory)
{
    assert(m_pGdbSrvControllerImpl != nullptr);
    return m_pGdbSrvControllerImpl->ReadMemory(address, size, isPhysicalMemory);
}
